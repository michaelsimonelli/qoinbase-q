...
.scm.cfrm:{$[not .scm.rcvt x;{{?[.ut.isNull'[x];(x#:)#("",:);]x}.scm.rcv2str each x};]x};
//.scm.rcvt:{t:{$[.ut.isGList x;.scm.rcvt;type]x}'[x];first $[1<count t;{(0h,x)({min x=/:y}.(0 1) cut x)};]t};
.scm.rcvt:{t:{$[.ut.isGList x;.scm.rcvt;type]x}'[x];first $[1<count t;{(0h,x)({b:min x=/:y;$[count b; b; 0b]}[x 0;x _ 1])};]t};
.scm.rcv2str:{if[10h ~ .scm.rcvt x; :x]; string x};
